...
function filterByTags(post) {
  ...
  const matched = filterTags.some(ft => postTags.some(pt => pt.includes(ft)));
  if (matched) {
...
function filterPromoted(post) {
  ...
  const promoted = post.find(S.promotedIndicator).length > 0 ||
...
function calculateSpamScore(posts) {
  ...
  const validPosts = posts.filter((p) => p.creationTs && typeof p.creationTs === 'number');
  if (validPosts.length < CONSTANTS.MIN_POSTS_FOR_SPAM_CHECK) {
    if (posts.length >= CONSTANTS.MIN_POSTS_FOR_SPAM_CHECK) {
      log(`Spam score: ${posts.length - validPosts.length} of ${posts.length} posts had invalid timestamps, reducing valid posts below threshold`);
    }
    return null;
  }
