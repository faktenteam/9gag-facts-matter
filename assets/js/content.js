...
if (typeof $ === "undefined") {
  console.error(
    "9GAG facts matter: jQuery/Cash.js is not loaded. Extension cannot function.",
  );
...
const postIntersectionObserver = new IntersectionObserver(
  (entries) => {
...
function reprocessAllPosts() {
  $(S.processed).each((i, el) => {
    ...
    $el
      .removeClass(
        "pf-processed filtering hidden hidden-spammer hidden-min-days hidden-keyword hidden-tag hidden-promoted hidden-blocked hidden-media",
      )
      .show();
...
function restoreScrollPosition() {
  ...
  const interval = setInterval(() => {
    ...
    if (
      !restored &&
      saved.scrollY > 0 &&
      document.body.scrollHeight >= saved.scrollY
    ) {
...
reapplyObserver.observe(document.querySelector(S.container) || document.body, {
  childList: true,
  subtree: true,
});
  ...
  chrome.storage.local.get(null, (items) => {
    const expired = Object.keys(items).filter((k) => {
      ...
      return (
        !entry ||
        !entry.timestamp ||
        now - entry.timestamp >= CONSTANTS.CACHE_DURATION
      );
    });
...
async function processPost(post) {
  ...
  try {
    ...
    if (!username) {
      log(
        "Could not extract username for post:",
        articleId,
